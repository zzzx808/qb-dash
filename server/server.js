...
function createPool() {
  ...
  let hostname = "";
  try {
    hostname = new URL(conn).hostname || "";
  } catch {
    hostname = "";
  }
  ...
  const isLocal =
    ...
    conn.includes("socket:");

  // Render's INTERNAL database host can be a short private hostname like `dpg-...-a`
  // (no dots). In that case, TLS is typically not required/available. For external
  // hosts (Render external URLs, Neon, Supabase, etc.), enable TLS.
  const isShortPrivateHost = hostname && !hostname.includes(".");
  const ssl = isLocal || isShortPrivateHost ? false : { rejectUnauthorized: false };
