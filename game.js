...
async function clearLeaderboard() {
  ...
  if (mode === "remote") {
    try {
      const res = await withTimeout(fetch("/api/leaderboard", { method: "DELETE" }), REMOTE_TIMEOUT_MS);
      if (res.ok) {
        return;
      }
    } catch {
...
function defenderMove(defender, dt, allDefenders, bounds, now) {
  ...
  if (defender.type === "chaser") {
    ...
    const toHome = normalizedOr(defender.homeX - defender.x, defender.homeY - defender.y, 0, 0);
...
field.addEventListener("mousemove", (event) => {
  ...
  const rect = fieldRect();
  state.qb.x = clamp(state.qb.x + dx, 12, rect.width - 12);
  state.qb.y = clamp(state.qb.y + dy, 12, rect.height - 12);
  renderQB();
});
